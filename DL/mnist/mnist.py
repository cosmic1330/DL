...
x_train = np.reshape(x_train, [-1, input_size]).astype('float32') # 先將原本28*28的2維數字影像，以reshape()轉換成1維向量，再以astype()轉換為float
x_train = x_train / 255 # normal
x_test = np.reshape(x_test, [-1, input_size]).astype('float32')
x_test = x_test / 255 # normal
...
# 儲存model和結果
model.save("./DL/mnist.h5",history.history)
np.save('./DL/mnist_history.npy',history.history)
